      ...
      <div className="container-x grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr] gap-12 items-center">
        ...
        <div className="fade-up text-center lg:text-left">
          <span className="glass inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full text-[13px] text-gold-400 mb-5">
            <AutoAwesomeIcon sx={{ fontSize: 15 }} /> {t("hero.badge")}
          </span>
          <h1 className="font-display text-[clamp(2.1rem,5vw,3.6rem)] leading-[1.1] font-bold m-0">
            {t("hero.title.a")} <span className="gold-text">{t("hero.title.stars")}</span> {t("hero.title.b")}
          </h1>
          ...
          <div className="flex gap-7 mt-7 flex-wrap justify-center lg:justify-start">
            {[
              ["50L+", t("hero.stat.kundlis")],
              ["1200+", t("hero.stat.astrologers")],
              ["4.9★", t("hero.stat.rating")],
            ].map(([n, l]) => (
              <div key={l}>
        ...
        <div className="glass card-hover fade-up rounded-[22px] p-5 sm:p-7 relative overflow-hidden">
          ...
          {!result ? (
            <>
              <h3 className="font-display text-[22px] font-semibold mt-0 mb-1">{t("hero.form.title")}</h3>
              <p className="text-ink-dim text-sm mt-0 mb-5">{t("hero.form.sub")}</p>
              <form onSubmit={onSubmit} className="grid gap-3.5">
                <label className="grid gap-1.5 text-left">
                  <span className="text-[13px] text-ink-dim">{t("hero.form.name")}</span>
                  <input value={form.name} onChange={(e) => update("name", e.target.value)} placeholder={t("hero.form.namePh")} className={inputClass} />
                </label>
                <div className="grid grid-cols-2 gap-3.5">
                  <label className="grid gap-1.5 text-left">
                    <span className="text-[13px] text-ink-dim">{t("hero.form.dob")}</span>
                    <input type="date" required value={form.date} onChange={(e) => update("date", e.target.value)} className={inputClass} />
                  </label>
                  <label className="grid gap-1.5 text-left">
                    <span className="text-[13px] text-ink-dim">{t("hero.form.tob")}</span>
                    <input type="time" value={form.time} onChange={(e) => update("time", e.target.value)} className={inputClass} />
                  </label>
                </div>
                <label className="grid gap-1.5 text-left">
                  <span className="text-[13px] text-ink-dim">{t("hero.form.place")}</span>
                  <input value={form.place} onChange={(e) => update("place", e.target.value)} placeholder={t("hero.form.placePh")} className={inputClass} />
                </label>
                ...
                <button type="submit" disabled={busy}
                  ...
                  {busy ? t("hero.form.busy") : (<>{t("hero.form.generate")} <ArrowForwardIcon sx={{ fontSize: 18 }} /></>)}
                </button>
                <p className="text-ink-dim text-[11px] text-center m-0">{t("hero.form.tip")}</p>
              </form>
            </>
          ) : (
            <div className="text-center py-2.5">
              ...
              <p className="text-ink-dim mt-0">{t("hero.result.moon")}</p>
              ...
              <p className="text-ink-dim text-[12px] mt-3 mb-0">{t("hero.result.preview")}</p>
              <div className="flex flex-wrap items-center justify-center gap-2 mt-3">
                <button onClick={() => router.push(user ? "/kundli" : "/register")}
                  ...
                  <AutoAwesomeIcon sx={{ fontSize: 18 }} /> {t("hero.result.full")}
                </button>
                <button onClick={() => setResult(null)}
                  ...
                  <ArrowBackIcon sx={{ fontSize: 18 }} /> {t("hero.result.another")}
                </button>
              </div>
            </div>
          )}
